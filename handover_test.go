...
var _ = Describe("Handover", func() {
	...
	It("client handover", func() {
		...
		Expect(err).ToNot(HaveOccurred())
		handoverState, err := clientSession.Handover(true)
		Expect(err).ToNot(HaveOccurred())
		migratedClientSession, err := RestoreSessionFromHandoverState(handoverState, protocol.PerspectiveClient, nil, "cloned")
		Expect(err).ToNot(HaveOccurred())
