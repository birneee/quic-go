...
type Stream struct {
	StreamID int64 `msg:"stream_id" json:"stream_id"`
	// in byte;
	// only set for write streams;
	// offset for writing to write queue;
	WriteOffset *int64 `msg:"write_offset,omitempty" json:"write_offset,omitempty"`
	// in byte;
	// only set for write streams if fin is written;
	WriteFin *int64 `msg:"write_fin,omitempty" json:"write_fin,omitempty"`
	// in byte;
	// only set for write streams;
	// also required for sending RESET_STREAM frames;
	WriteMaxData *int64 `msg:"write_max_data,omitempty" json:"write_max_data,omitempty"`
	// in byte;
	// only set for write streams;
	// offset until stream data is acknowledged
	WriteAck *int64 `msg:"write_ack,omitempty" json:"write_ack,omitempty"`
	// only set for write streams;
	...
	ReadFin *int64 `msg:"read_fin,omitempty" json:"read_fin,omitempty"`
	// in byte;
	// only set for read streams;
	ReadMaxData *int64 `msg:"read_max_data,omitempty" json:"read_max_data,omitempty"`
	// only set for read streams;
