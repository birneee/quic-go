...
func (s *connection) run() error {
	...
	if !s.cloned { // skip handshake for smaq restored connections
...
func (s *connection) handleIgnoredAddressPacket(rp receivedPacket) {
	if s.tracer != nil {
		s.tracer.DroppedPacket(logging.PacketTypeNotDetermined, -1, rp.Size(), logging.PacketDropHquicIgnoreRemoteAddress)
	}
...
func Restore(transport *Transport, state *qstate.Connection, restoreConf *ConnectionRestoreConfig) (Connection, *RestoredStreams, error) {
	...
	s := &connection{
		...
		ctx:                   ctx,
		ctxCancel:             ctxCancel,
		handshakeCompleteChan: closedChan,
