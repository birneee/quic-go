...
var _ = Describe("SentPacketHandler", func() {
	...
	JustBeforeEach(func() {
		...
		handler = newSentPacketHandler(42, protocol.InitialPacketSizeIPv4, protocol.DefaultInitialCongestionWindow, protocol.DefaultMinCongestionWindow, protocol.DefaultMaxCongestionWindow, protocol.DefaultInitialSlowStartThreshold, protocol.DefaultMinSlowStartThreshold, protocol.DefaultMaxSlowStartThreshold, rttStats, false, perspective, false, 0, nil, utils.DefaultLogger)
		streamFrame = wire.StreamFrame{
			StreamID: 5,
			Data:     []byte{0x13, 0x37},
		}
	...
	Context("amplification limit, for the server, with validated address", func() {
		JustBeforeEach(func() {
			rttStats := utils.NewRTTStats()
			handler = newSentPacketHandler(42, protocol.InitialPacketSizeIPv4, protocol.DefaultInitialCongestionWindow, protocol.DefaultMinCongestionWindow, protocol.DefaultMaxCongestionWindow, protocol.DefaultInitialSlowStartThreshold, protocol.DefaultMinSlowStartThreshold, protocol.DefaultMaxSlowStartThreshold, rttStats, true, perspective, false, 0, nil, utils.DefaultLogger)
		})
